...
export const uiCopy = {
  en: {
    ...
    articleMissingBody:
      'The requested article is not available in the current article stream. Return to the article list to keep reading the latest knowledge and application notes.',
    productMissingBody:
      'The requested machine entry is not available in the current catalog. Return to the catalog to continue browsing the featured equipment range.',
    ...
    formResponseNote: 'Inquiry details help route the right follow-up through email and messaging channels.',
  ...
  zh: {
    ...
    productMissingBody: '当前产品目录中没有这条设备信息。请返回产品目录，继续浏览重点设备。',
    ...
    formEmailPlaceholder: 'name@company.com',
    formSubmit: '提交需求',
    formResponseNote: '你填写的询盘信息将帮助后续通过邮箱或即时通讯进行准确跟进。',
  ...
  ar: {
    ...
    productMissingBody: 'هذا الإدخال الخاص بالآلة غير متاح في الكتالوج الحالي. ارجع إلى الكتالوج لمتابعة تصفح المعدات الرئيسية المعروضة.',
    ...
    formResponseNote: 'تفاصيل الاستفسار تساعد على توجيه المتابعة المناسبة عبر البريد أو قنوات المراسلة.',
  ...
  ru: {
    ...
    articleMissingBody:
      'Запрошенная статья недоступна в текущем потоке статей. Вернитесь в раздел статей, чтобы продолжить чтение материалов о знаниях и применении.',
    productMissingBody:
      'Запрошенная машина недоступна в текущем каталоге. Вернитесь в каталог, чтобы продолжить просмотр ключевого оборудования.',
    ...
    formResponseNote: 'Детали запроса помогают направить правильное продолжение по email или в каналах связи.',
